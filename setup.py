...
with open(os.path.join(basepath, "sqlalchemy_pymssql_sybase", "__init__.py")) as finp:
    VERSION = re.match(r'.*__version__ = "(.*?)"', finp.read(), re.S).group(1)

with open(os.path.join(basepath, "requirements.txt")) as finp:
    REQUIREMENTS = [pkg for pkg in (line.split("#")[0].strip() for line in finp) if pkg]

with open(os.path.join(basepath, "README.rst")) as finp:
    README = finp.read()
